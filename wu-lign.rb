...
def repair_line(line)
  # $stderr.puts "repairing line: "
  # $stderr.puts line
  repaired_line = []
  line.each_char do |char|
    repaired_line << (char.valid_encoding? ? char : REPLACEMENT_CHAR)
  end
  repaired_line.join
end
...
def to_tuple(line)
  line = repair_line(line) unless line.valid_encoding?
  line.chomp.split("\t").map{|s| s.strip }
end
...
FORMAT_GUESSING_LINES.times do
  ...
  row.each_with_index{|col,i|
    next if skip_col[i]
    # Allow the first row to be text (headers)
...
format = maxw.zip(col_types, col_minmag, col_maxmag, FORMAT_SPECS).map do |width, type, minmag, maxmag, default|
  ...
  case type
  when :mixed, nil then lambda{|ss| "%-#{width}s" % ss }
  when :str        then lambda{|ss| "%-#{width}s" % ss }
  when :int        then lambda{|ss| ss = ss.gsub(/[^\d\-\+]+/, "");
      (ss.empty? ? ("%-#{width}s"%ss) : ("%-#{width}d" % ss.to_i)) }
  when :float      then lambda{|ss| ss = ss.gsub(/[^\d\.eE\-\+]+/, '');
      (ss.empty? ? ("%-#{maxmag+minmag+2}s"%ss) : ("%#{maxmag+minmag+2}.#{minmag}f" % ss.to_f)) }
...
if ENV['WULIGN_DEBUG'] == 'true'
  $stderr.puts "maxw:\t#{maxw.zip(col_types, col_minmag, col_maxmag).inspect}"
end
